...
function hackStage(stage) {
    ...
    if (input.value === codes[stage]) {
        document.getElementById(`${stage}-stage`).style.display = 'none';
        if (stage === 'firewall') {
            document.getElementById('captcha-stage').style.display = 'block';
            document.querySelector('#projects-challenge').style.display = 'none';
            document.querySelector('#projects .projects-grid').style.display = 'grid';
            gsap.from('#projects .projects-grid', { opacity: 0, y: 50, duration: 0.5 });
        } else if (stage === 'captcha') {
            document.getElementById('network-stage').style.display = 'block';
            document.querySelector('#skills-challenge').style.display = 'none';
            document.querySelector('#skills .skills-grid').style.display = 'block';
            gsap.from('#skills .skills-grid', { opacity: 0, y: 50, duration: 0.5 });
        } else if (stage === 'network') {
            document.querySelector('#certifications-challenge').style.display = 'none';
            document.querySelector('#certifications .certifications-grid').style.display = 'grid';
            gsap.from('#certifications .certifications-grid', { opacity: 0, y: 50, duration: 0.5 });
        }
...
function closeEasterEggModal() {
    gsap.to('#easter-egg-modal .modal-content', {
        ...
        opacity: 0,
        duration: 0.4,
        onComplete: () => document.getElementById('easter-egg-modal').style.display = 'none'
    });
}
...
function closeRecruitModal() {
    gsap.to('#recruit-modal .modal-content', {
        ...
        opacity: 0,
        duration: 0.4,
        onComplete: () => document.getElementById('recruit-modal').style.display = 'none'
    });
}

function submitRecruit() {
    const codename = document.querySelector('#recruit-modal input').value.trim();
    if (codename) {
...
// Add Recruit Button to Contact Section
document.querySelector('#contact .contact-container').insertAdjacentHTML('beforeend', '<button class="recruit-button" onclick="openRecruitModal()">Join Secret Network</button>');
...
commands['decrypt_skills'] = () => {
    document.querySelector('#skills-challenge').style.display = 'none';
    document.querySelector('#skills .skills-grid').style.display = 'block';
    gsap.from('#skills .skills-grid', { opacity: 0, y: 50, duration: 0.5 });
};

commands['connect_to_mohamed'] = () => {
...
// Override Project Card Click for Fake Breach
document.querySelectorAll('.project-card').forEach(card => {
